#!/usr/bin/env python3
"""
2-YEAR BACKTEST (1H) - ALL MAJOR PAIRS
...
class MultiPairBacktest1H_LongTerm:
    """2-year backtest across all major pairs."""
    ...
    def fetch_all_pairs(self):
        ...
        print("📥 Fetching 2 YEARS of 1H data for all major pairs...")
        print("-" * 80)
        ...
        for pair in self.MAJOR_PAIRS:
            try:
                ...
                candles = len(df)
                days = candles / 24
                logger.info(f"✅ {pair}: {candles} candles ({days:.0f} trading days)")
    ...
    def run_backtest(self):
        """Run multi-pair 1H backtest over 2 years."""
        print("🔄 Running 2-YEAR backtest across all pairs...\n")
        ...
        print(f"Processing {max_candles:,} candles per pair (~{max_candles/24/365:.1f} years)...\n")
        ...
        for candle_idx in range(lookback, max_candles):
            if candle_idx % 5000 == 0:
                print(f"  Progress: {candle_idx:,}/{max_candles:,} candles ({candle_idx/max_candles*100:.1f}%)")
...
def main():
    print("\n" + "="*80)
    print("2-YEAR 1H BACKTEST - ALL MAJOR PAIRS")
    print("="*80 + "\n")

    backtester = MultiPairBacktest1H_LongTerm(years_back=2)
    ...
    print("\n" + "="*80)
    print("BACKTEST RESULTS - 2 YEARS")
    print("="*80)
    print(f"\n📊 Trade Statistics:")
    ...
    print(f"  Largest Win: {stats['largest_win']:+.0f}p")
    print(f"  Largest Loss: {stats['largest_loss']:+.0f}p")
    ...
    if stats['win_rate'] >= 60 and profit_15days >= 150:
        print(f"  ✅ CAN HIT $200 IN 15 DAYS (CONFIRMED over 2 years!)")
    elif stats['win_rate'] >= 55 and profit_30days >= 150:
        print(f"  ✅ CAN HIT $200 IN 30 DAYS")
    elif stats['win_rate'] >= 50:
        print(f"  ⚠️  PROFITABLE BUT SLOWER (${20 + profit_30days:.0f} in 30 days)")
    else:
        print(f"  ❌ NOT PROFITABLE LONG-TERM")

    print(f"\n🔍 Statistical Confidence:")
    print(f"  Sample Size: {stats['total_trades']:,} trades over 2 years")
    print(f"  Result Reliability: {'VERY HIGH' if stats['total_trades'] > 1000 else 'HIGH' if stats['total_trades'] > 100 else 'MODERATE'}")
    print(f"  This proves the strategy {'IS ROBUST' if stats['total_trades'] > 500 else 'NEEDS MORE DATA'}")
    ...
    for pair in sorted(pair_stats.keys(), key=lambda p: pair_stats[p]['pips'], reverse=True):
        ...
        daily_avg = data['pips'] / len(backtester.daily_trades) * 0.10
        print(f"  {pair:8s}: {data['trades']:4d} trades, {wr:5.1f}% WR, {data['pips']:+7.0f}p, ${daily_avg:+6.2f}/day")
